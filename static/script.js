...
/* ── sendMessage'a limit_reached eventi ekle */
const _origSend = window.sendMessage;
window.sendMessage = async function() {
  // Limit bilgisini önceden kontrol et
  if (currentUser && !currentUser.isAdmin) {
    const rem = (currentUser.limit ?? 10) - (currentUser.used ?? 0);
    if (rem <= 0) {
      showLimitModal(currentUser.used, currentUser.limit);
      return;
    }
  }
  // Orijinal sendMessage'ı çalıştır; limit_reached event'ini dinle
  await _origSend.call(this, ...arguments);
};

// SSE'de limit_reached event'ini yakala
const _origFetch = window.fetch;
// sendMessage içindeki SSE handler'ını patch et
// Bu yaklaşım: script.js'deki evt handler'ına limit_reached ekle
const _patchSSE = setInterval(() => {
  // sendMessage yeniden tanımlanmışsa interval'i temizle
  clearInterval(_patchSSE);
}, 100);
