...
window.doLogout = async function() {
  ...
  currentUser = null;
  profile = { name: 'Kullanıcı', avatar: '', customPrompt: '' };
  localStorage.removeItem('ep');
  window.location.reload();
};

/* ── Admin modal satırını güncelle ─────── */
function _updateModalAdminRow() {
  const row = document.getElementById('modalAdminRow');
  if (row) row.style.display = (currentUser && currentUser.isAdmin) ? 'block' : 'none';
}
...
async function sendMessage() {
  ...
  // Limit ön kontrolü (frontend)
  if (currentUser && !currentUser.isAdmin) {
    const rem = (currentUser.limit ?? 10) - (currentUser.used ?? 0);
    if (rem <= 0) {
      if (typeof showLimitModal === 'function') showLimitModal(currentUser.used, currentUser.limit);
      return;
    }
  }
  ...
  try {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (evt.type === 'search') {
          ...
          // Sayacı güncelle
          if (currentUser && !currentUser.isAdmin) {
            currentUser.used = (currentUser.used ?? 0) + 1;
            if (typeof updateCounter === 'function') updateCounter(currentUser.used, currentUser.limit ?? 10);
          }
        } else if (evt.type === 'limit_reached') {
          removeTyping();
          if (typeof showLimitModal === 'function') {
            showLimitModal(evt.used ?? 0, evt.limit ?? 10);
          }
          console.warn('[Chat] Günlük limit aşıldı:', evt);
    ...
    if (err.name !== 'AbortError') {
      removeTyping();
      console.error('[Chat] Fetch/SSE hatası:', err);
      appendMsg('Bağlantı hatası. Flask çalışıyor mu? (Konsolu kontrol et)', 'bot', true);
    }
  }
...
async function doLogin() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      if (data.isAdmin) { showToast('👑 Admin paneline yönlendiriliyorsun…'); setTimeout(() => window.location.href = '/admin', 800); }
    } else {
...
/* ═══════════════════════════════════════
   AUTH V2 — /me session check + counter
   + register + limit modal
═══════════════════════════════════════ */

/* ── Auth State ───────────────────────── */
let currentUser = null;

/* ── Mesaj sayacı (TTS yerine) ─────────── */
function updateCounter(used, limit) {
  ...
  if (!counter) {
    ...
    counter.style.cssText = [
      ...
      'border-radius:var(--rf)','padding:4px 10px','transition:all 0.2s',
    ].join(';');
    const ttsBtn = document.getElementById('btnTts');
    if (ttsBtn) ttsBtn.replaceWith(counter);
    else right.prepend(counter);
  }
  const remaining = Math.max(0, limit - used);
  const pct   = limit > 0 ? (remaining / limit) : 1;
  const color = pct > 0.5 ? 'var(--ok)' : pct > 0.2 ? 'var(--warn)' : 'var(--err)';
  ...
  counter.innerHTML =
    `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
     ...
     <span>Kalan: <strong>${remaining}/${limit}</strong></span>`;
  counter.title = `${used} mesaj kullandın, ${remaining} kaldı`;
}

/* ── Session kontrolü (sayfa yüklenince) */
async function checkSession() {
  try {
    ...
    if (data.ok) {
      ...
      renderProfile(); renderChatUI(); setTier('balanced');
      if (!data.isAdmin) updateCounter(data.used ?? 0, data.limit ?? 10);
      if (data.isAdmin) { showToast('👑 Admin paneline yönlendiriliyorsun…'); setTimeout(() => window.location.href = '/admin', 800); }
      return true;
    }
  } catch {}
...
window.doLogin = async function() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      if (errEl) { errEl.textContent = data.error || 'Hatalı giriş'; errEl.classList.add('show'); }
    }
  } catch {
    if (errEl) { errEl.textContent = 'Sunucuya bağlanılamadı'; errEl.classList.add('show'); }
  }
};
...
window.doRegister = async function() {
  ...
  if (errEl) errEl.classList.remove('show');
  if (!u || !e || !p) {
    if (errEl) { errEl.textContent = 'Tüm alanları doldur'; errEl.classList.add('show'); }
    return;
  }
  try {
    ...
    if (data.ok) {
      ...
      renderProfile(); renderChatUI(); setTier('balanced');
      updateCounter(0, 10);
      showToast('✅ Kayıt başarılı, hoş geldin ' + data.username + '!');
    } else {
      if (errEl) { errEl.textContent = data.error || 'Kayıt başarısız'; errEl.classList.add('show'); }
    }
  } catch {
    if (errEl) { errEl.textContent = 'Sunucuya bağlanılamadı'; errEl.classList.add('show'); }
  }
...
window.showLimitModal = function(used, limit) {
  ...
  m.id = 'limitModal';
  m.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(16px);z-index:400;display:flex;align-items:center;justify-content:center;padding:16px';
  m.innerHTML = `
    <div style="background:var(--s1);border:1px solid var(--b2);border-radius:20px;padding:32px;max-width:380px;width:100%;text-align:center;box-shadow:0 32px 80px rgba(0,0,0,0.6);animation:modalIn 0.3s var(--spring)">
      <div style="font-size:40px;margin-bottom:12px">⚡</div>
      <div style="font-size:18px;font-weight:700;color:var(--t0);margin-bottom:8px">Günlük limitine ulaştın</div>
      <div style="font-size:13px;color:var(--t3);margin-bottom:22px;line-height:1.7">
        Bugün <strong style="color:var(--warn)">${used}</strong> mesaj kullandın.<br>
        Limitin <strong>${limit}</strong>. Daha fazlası için <strong style="color:var(--ac2)">Premium</strong>'a geç.
      </div>
      <div style="display:flex;gap:8px;justify-content:center">
        <button onclick="document.getElementById('limitModal').remove()"
          style="padding:9px 18px;border-radius:10px;border:1px solid var(--b2);background:transparent;color:var(--t2);font-size:13px;cursor:pointer">
          Kapat
        </button>
        <a href="https://discord.com/users/emirai" target="_blank" style="text-decoration:none">
          <button style="padding:9px 18px;border-radius:10px;border:none;background:linear-gradient(135deg,var(--ac),var(--ac2));color:#fff;font-size:13px;font-weight:600;cursor:pointer">
            💎 Premium Al
          </button>
        </a>
      </div>
...
/* ── sendMessage limit check (düzeltildi) ── */
// NOT: window.sendMessage override yerine sendMessage doğrudan güncellendi.
// limit_reached event'i artık sendMessage içindeki SSE döngüsünde yakalanıyor.

/* ── Init override: önce session kontrol ─ */
(async () => {
  const loggedIn = await checkSession();
  if (!loggedIn) {
    // Session yok, UI'yı başlat ama login ekranı görünsün
    setTier('balanced');
  }
})();
